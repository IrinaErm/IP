...
function hideDivsColumns(slideIndex) {
	...
	for(let i = slideIndex*DIVS; i < slideIndex*DIVS + DIVS; i++) {
		$(document).ready(function(){
			if (i % 3 == 0) {
				$(s[i]).animate({height:"0px",opacity:"1"},700 + 50*(i/3));
			}
			else if (i % 5 == 0) {
				$(s[i]).animate({height:"0px",opacity:"1"},950 - 30*(i%3));
			}
			else if (i % 4 == 0) {
				$(s[i]).animate({height:"0px",opacity:"1"},950 - 25*(i%5));
			}
			else {
				$(s[i]).animate({height:"0px",opacity:"1"},1000 - 25*(i/3));
			}
		});			
	}
}
...
function showDivs() {
	...
	for(var i = slideIndex*DIVS; i < slideIndex*DIVS + DIVS; i++) {
		...
		$(s[i]).css('height', '100%');
		if(hide) {
			$(s[i]).css('background-color', '#F7F7F7');
			$(s[i]).css('background-image', 'none');
			$(s[i]).css('border', '1px solid #F7F7F7');
		}
		else {
			$(s[i]).css('background-image', 'url("pics/'+(lastSlide+1)+'.jpg")');
			$(s[i]).css('border-left', '1px solid transparent');
			$(s[i]).css('border-right', '1px solid transparent');
			$(s[i]).css('border-bottom', '1px solid transparent');
		}	
	}	
}
